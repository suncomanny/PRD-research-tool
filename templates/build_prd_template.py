...
intro_lines = [
    ...
    ("", instr_body),
    ("  If you have Stackline Atlas data for the subcategory you\u2019re researching,", instr_body),
    ...
    ("    b. Navigate to the relevant product segment for your subcategory", instr_body),
    ...
    ("    e. Export the Summary table and Traffic trend for the segment", instr_body),
    ('      \u2022 Summary CSV \u2014 product-level revenue, units sold, retail price, brand share', instr_body),
    ('      \u2022 Traffic CSV \u2014 total traffic trend used to derive conversion', instr_body),
    ('      \u2022 Sales CSV \u2014 optional secondary file until we confirm its schema is consistent', instr_body),
    ("", instr_body),
    ("  Step 2: Rename the files using this convention", instr_bold),
    ("    Format:  Stackline_[StacklineSegment]_[YYYY-MM]_[type].csv", instr_body),
    ("", instr_body),
    ("    [StacklineSegment] = The Stackline segment label or saved alias (replace spaces with _)", instr_body),
    ("    [YYYY-MM]  = Year and month of the export (e.g., 2026-04)", instr_body),
    ("    [type]     = summary  OR  traffic  OR  sales", instr_body),
    ("", instr_body),
    ("    Examples:", instr_bold),
    ("      Stackline_Ceiling_Panels_2026-04_summary.csv", instr_body),
    ("      Stackline_Ceiling_Panels_2026-04_traffic.csv", instr_body),
    ("      Stackline_Wraparound_LED_2026-04_summary.csv", instr_body),
    ("      Stackline_Wraparound_LED_2026-04_traffic.csv", instr_body),
    ("", instr_body),
    ("  Step 3: Upload to SharePoint", instr_bold),
    ("    Upload both files to the Product SharePoint site:", instr_body),
    ("      Manny Tools / PRD Research / Stackline Data /", instr_body),
    ("", instr_body),
    ...
    ("    automatically find the newest matching file bundle for the segment.", instr_body),
    ("", instr_body),
    ("  Note: If multiple exports exist for the same segment, the tool picks", instr_body),
